...
class ParkingServices {
  ...
  Future<List<ParkingDb>?> getParkings(
      {String? parkingName,
      int? amountOfSpots,
      int? takenSpots,
      double? totalIncome,
    ...
    if (parkingName != null) {
      parkingList =
          parkingList.where((park) => park.name.contains(parkingName)).toList();
    } else if (amountOfSpots != null) {
      parkingList = parkingList
          .where((park) => park.spots.length == amountOfSpots)
          .toList();
    } else if (totalIncome != null) {
      parkingList =
          parkingList.where((park) => park.income == totalIncome).toList();
    } else if (todayIncome != null) {
      parkingList =
          parkingList.where((park) => park.dailyIncome == todayIncome).toList();
    }
  ...
  Future<double> setIncome(String parkingName) async {
    ...
    DatabaseReference _dbRef2 = FirebaseDatabase.instance
        .ref()
        .child('parking_history')
        .child(parkingName);
    DataSnapshot parkingSnapshot = await _dbRef2.get();

    if (parkingSnapshot.value != null &&
        parkingSnapshot.value is Map<String, dynamic>) {
      Map<String, dynamic> parkingData =
          parkingSnapshot.value as Map<String, dynamic>;
  ...
  Future<void> startParking(int spotId, String parkingName,
      String? registration, int lvl, Layover ticket, String ticketKey) async {
    ...
    await _dbRef
        .child(parkingName)
        .child('spots')
        .child(spotId.toString())
        .set(spotMap);
    ticketService.addTicket(ticket, ticketKey);
  }

  Future<void> moveFromParking(
      int spotId, String parkingName, double cost, String userLogin) async {
    DataSnapshot dataSnapshot = await _dbRef
        .child(parkingName)
        .child('spots')
        .child(spotId.toString())
        .get();
    ...
    if (spotData != null) {
      ...
      parkHistory.addToParkingHistory(parkingName, spotId.toString(),
          registrationNumber, DateTime.parse(date), DateTime.now(), cost);
      //print(parkingName);
      //print(spotData);
      await _dbRef
          .child(parkingName)
